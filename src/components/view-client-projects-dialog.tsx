...
import type { Client, Project, TeamMember, ProjectStatus, Priority } from "@/lib/data";
import { format } from "date-fns";
import { TrendingUp, TrendingDown, Calendar, Users, DollarSign, Briefcase, MapPin } from "lucide-react";

interface ViewClientProjectsDialogProps {
  isOpen: boolean;
...
export function ViewClientProjectsDialog({
  isOpen,
  onOpenChange,
  client,
  projects,
  teamMembers,
}: ViewClientProjectsDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-2xl">Projects for {client.name}</DialogTitle>
          <DialogDescription>
            A summary of all projects associated with this client.
          </DialogDescription>
        </DialogHeader>
          ...
          {clientProjects.length > 0 ? (
            <div className="space-y-4 py-2">
              {clientProjects.map((project) => {
                ...
                const originatingLocation = client.locations.find(l => l.id === project.clientLocationId);
                        ...
                        <div>
                            ...
                            {originatingLocation && (
                                <div className="flex items-center gap-1.5 text-sm text-muted-foreground mt-1">
                                    ...
                                    <span>From: {originatingLocation.name} Office</span>
                                </div>
                            )}
                        </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-muted-foreground">This client does not have any projects in the system yet.</p>
            </div>
          )}
        </ScrollArea>
        </div>
