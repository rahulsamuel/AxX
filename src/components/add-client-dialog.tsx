...
import { Building, Plus, Trash2, Star } from "lucide-react";
import { cn } from "@/lib/utils";
import { addClient, getClientTypes } from "@/lib/database";
...
export function AddClientDialog({ isOpen, onOpenChange, onClientAdded }: AddClientDialogProps) {
  ...
  const form = useForm<ClientFormValues>({
    ...
    defaultValues: {
      name: "",
      type: "",
      locations: [defaultLocationValues],
  ...
  React.useEffect(() => {
    if (isOpen) {
        getClientTypes().then(types => {
          setClientTypes(types);
          if (types.length > 0 && form.getValues('type') === '') {
              form.setValue('type', types[0]);
          }
        });
    } else {
      form.reset();
                              ...
                              <div key={fieldItem.id} className="p-4 border rounded-lg relative">
                                <div className="absolute top-3 right-3 flex items-center gap-2">
                                     <FormItem className="flex items-center space-x-2 space-y-0">
                                        ...
                                        <FormLabel htmlFor={`primary-${index}`} className="flex items-center gap-1.5 cursor-pointer text-sm font-medium">
                                            <Star className={cn("h-4 w-4", form.getValues('primaryLocationIndex') == String(index) ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground')} />
                                            Primary
                                        </FormLabel>
                                    </FormItem>
                                  {fields.length > 1 && (
